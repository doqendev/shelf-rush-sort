...
- **Hand-authored curriculum, levels 1–15** (Sprints B + E): levels 1–10 teach one new model each (clear → second clear → empty-space → hidden reveal → mystery bag → timer → combinations → capstone); levels 11–15 are mastery levels that escalate via tighter timers and combined obstacles. Every level is solver-verified completable, collision-free, carries a player-facing teaching banner, and has solver-seeded star thresholds.
...
## Content note
The live runtime pack is `level_pack_vertical_slice.json` — a hand-authored 15-level curriculum (the full pack). The 300-level `level_pack_000.json` is CI-validated only (collision-free, solvable) and is **not** presented as human-tuned player-facing content.
